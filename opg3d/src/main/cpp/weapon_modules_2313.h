...
//   Merging after the original method returns is the first reliable point where
//   the controller and both lists are fully initialized.
...
// Entries already present are retained and List<T>.Contains prevents duplicates.
// -----------------------------------------------------------------------------
...
constexpr uint64_t kUiRecheckEvery = 24;
...
inline bool g_merge_error_reported = false;
...
inline MergeResult merge_catalog(void* self, void* field, const Managed& source) {
    ...
    if (full == nullptr) return result;
    ListApi full_api;
    if (!resolve_list_api(full, full_api) || !full_api.readable()) return result;
    result.source =
        reinterpret_cast<ListCountFn>(full_api.count_ptr)(full, full_api.count_info);
    if (result.source <= 0 || result.source > kMaxCatalogEntries) return result;

    void* owned = read_object_field(self, field);
    if (owned == nullptr) {
        write_object_field(self, field, full);
        owned = read_object_field(self, field);
        result.before = 0;
        result.after = list_count(owned);
        result.added = result.after > 0 ? result.after : 0;
        result.ok = owned != nullptr && result.after == result.source;
        return result;
    }
    ...
    ListApi owned_api;
    if (!resolve_list_api(owned, owned_api) || !owned_api.writable()) return result;
...
inline void report_merge(const char* reason, const char* what,
    ...
    if (result.added > 0) {
        LOGI("23.1.3-modules: %s: %s source=%d owned=%d +%d -> %d", reason, what,
             result.source, result.before, result.added, result.after);
    }
    if (result.ok && !complete_reported) {
        complete_reported = true;
        LOGI("23.1.3-modules: %s inventory complete (%d/%d)", what, result.after,
             result.source);
    } else if (!result.ok && result.source > 0 && !g_merge_error_reported) {
        g_merge_error_reported = true;
        LOGE("23.1.3-modules: %s: incomplete %s merge "
             "(source=%d before=%d added=%d after=%d)",
             reason, what, result.source, result.before, result.added, result.after);
    }
}

inline void grant(void* self, const char* reason) {
    if (self == nullptr || g_in_grant) return;
    g_in_grant = true;
    g_controller = self;
    const MergeResult modules =
        merge_catalog(self, g_field_owned_modules, g_catalog_modules);
    const MergeResult sets = merge_catalog(self, g_field_owned_sets, g_catalog_sets);
...
inline void on_instance_created_hook(void* self, void* method) {
    ...
    grant(self, "OnInstanceCreated");
}

inline void reload_hook(void* self, void* method) {
    if (g_orig_reload != nullptr) g_orig_reload(self, method);
    grant(self, "profile reload");
}

inline void* selected_hook(void* self, void* method) {
    void* result = nullptr;
    if (g_orig_selected != nullptr) result = g_orig_selected(self, method);
    if ((g_selected_calls++ % kUiRecheckEvery) == 0) {
        grant(self, "module UI");
    }
    return result;
}
...
inline int32_t current_level_hook(void* self, void* method) {
    ...
    if (g_controller != nullptr && !g_in_grant &&
        ...
        grant(g_controller, "level fallback");
    }
    const int32_t level = original >= kTargetModuleLevel
                              ? original
                              : kTargetModuleLevel;
    if ((g_level_calls % kLevelLogEvery) == 1) {
        LOGI("23.1.3-modules: displayed level %d -> %d (call %llu)", original, level,
...
inline bool install() {
    ...
    g_field_owned_modules =
        il2cpp::find_field(kNs, kControllerClass, kOwnedModulesField);
    ...
    if (g_field_owned_modules == nullptr || g_field_owned_sets == nullptr) {
        LOGE("23.1.3-modules: inventory fields unavailable "
             "(modules=%p sets=%p), module disabled",
    ...
    if (ok) {
        LOGI("23.1.3-modules: corrected grant armed (expect %d modules at level X)",
             kExpectedModules);
    } else {
