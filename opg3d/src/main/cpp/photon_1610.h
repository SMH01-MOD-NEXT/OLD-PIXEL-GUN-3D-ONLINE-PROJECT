...
// Photon/PUN compatibility for the supplied obfuscated PG3D 16.1.1 ARMv7
// build (libil2cpp.so SHA-256
// 2aab620cb58a597e86975a78ab20987e71685b507456707ed42fa63fad54032b).
//
// Names below are metadata names from that exact dump. They are intentionally
// kept in one versioned module: none of the unobfuscated 14.1.1 names are
// assumed to survive. The port changes only the connection route:
...
//     or connected, preserving the old project's proven dead-backend guard.
// Rooms, matchmaking, RPCs, callbacks and manual disconnects stay stock.
namespace photon_1610 {
namespace detail {

using MethodInfo = void;
using ManagedString = void;
using SelectAppIdFn = ManagedString* (*)(void* static_context,
                                         void* hidden_settings,
                                         const MethodInfo* method);
using SetUpPhotonFn = void (*)(void* static_context, void* hidden_settings,
                              const MethodInfo* method);
using ConnectUsingSettingsFn = bool (*)(void* static_context,
                                        ManagedString* game_version,
                                        const MethodInfo* method);
...
using StaticGetBoolFn = bool (*)(void* static_context,
                                 const MethodInfo* method);
using StaticSetBoolFn = void (*)(void* static_context, bool value,
                                 const MethodInfo* method);
...
bool resolve_method(const char* klass, const char* name, int args,
    ...
    if (method == nullptr || pointer == nullptr) {
        LOGE("16.1.1-photon: cannot resolve %s.%s/%d", klass, name, args);
        return false;
    }
    *function = pointer;
    *info = method;
...
int32_t connection_state() {
    ...
    return g_get_state != nullptr ? g_get_state(nullptr, g_state_info) : -1;
}

bool ensure_online_mode(void* static_context) {
    ...
    const bool before = g_get_offline(static_context, g_offline_get_info);
    if (before) {
        LOGW("16.1.1-photon: PUN offlineMode was set by the auth-scene "
             "fallback; clearing it before online connect");
        g_set_offline(static_context, false, g_offline_set_info);
    }
    const bool after = g_get_offline(static_context, g_offline_get_info);
    if (after) {
        LOGE("16.1.1-photon: PUN offlineMode stayed enabled; online connect "
...
bool force_cloud(const char* point) {
    ...
    if (app_length == 0u) {
        if (!g_empty_app_logged.exchange(true)) {
            LOGE("16.1.1-photon[%s]: PHOTON_APP_ID is empty", point);
        }
        return false;
    }
    ...
    if (settings == nullptr) {
        LOGW("16.1.1-photon[%s]: PhotonServerSettings is not ready", point);
        return false;
    }
    ManagedString* app_id = il2cpp::string_new != nullptr
                                ...
                                : nullptr;
    if (app_id == nullptr) {
        LOGE("16.1.1-photon[%s]: il2cpp_string_new failed", point);
        return false;
    }

    if (g_use_cloud == nullptr) {
        resolve_method("ServerSettings", kUseCloudMethod, 2,
    ...
    LOGI("16.1.1-photon[%s]: apply#%" PRIu32
    ...
    if (!ready) {
        LOGE("16.1.1-photon[%s]: PhotonCloud/eu route verification failed",
             point);
    }
    return ready;
#endif
...
ManagedString* hook_select_app_id(void* static_context, void* hidden_settings,
                                  const MethodInfo* method) {
    ...
    if (length != 0u && il2cpp::string_new != nullptr) {
        ...
        if (replacement != nullptr) {
            LOGI("16.1.1-photon: obfuscated Switcher AppID selector -> "
                 "configured credential (chars=%zu fnv=%08" PRIx32 ")",
                 length, fnv1a(PHOTON_APP_ID, length));
            return replacement;
        }
    }
    LOGW("16.1.1-photon: AppID selector passthrough (build secret missing "
         "or string allocation failed)");
    return g_select_app_id != nullptr
               ? g_select_app_id(static_context, hidden_settings, method)
               : nullptr;
}

void hook_set_up_photon(void* static_context, void* hidden_settings,
                        const MethodInfo* method) {
    if (g_set_up_photon != nullptr) {
        g_set_up_photon(static_context, hidden_settings, method);
    }
    force_cloud("Switcher.SetUpPhoton/end");
}

bool hook_connect_using_settings(void* static_context,
                                 ManagedString* game_version,
                                 const MethodInfo* method) {
    g_connect_started.store(true, std::memory_order_release);
    const bool online_mode = ensure_online_mode(static_context);
    const bool route = force_cloud("PUN.ConnectUsingSettings");
    LOGI("16.1.1-photon: ConnectUsingSettings begin state=%d "
         "onlineMode=%d route=%d",
         connection_state(), online_mode ? 1 : 0, route ? 1 : 0);
    if (g_connect_using_settings == nullptr) return false;
    const bool result =
        g_connect_using_settings(static_context, game_version, method);
    LOGI("16.1.1-photon: ConnectUsingSettings returned %d state=%d",
    ...
    if (g_connect_started.load(std::memory_order_acquire)) {
        ...
        if (state >= 2 && state != kDisconnectedState) {
            ...
            if (count <= 8u || count % 120u == 0u) {
                LOGW("16.1.1-photon: skipped FriendsController.Update #%" PRIu32
                     " state=%d; dead social/backend tick cannot disconnect PUN",
                     count, state);
            }
            return;
        }
...
inline bool install_hooks() {
    ...
    LOGI("16.1.1-photon: installed %d hooks "
         "(appid=%s connect=%s backend-guard=%s; "
         "PUN class and methods resolved by obfuscated metadata names)",
         installed, app_id ? "OK" : "FAILED",
         connect ? "OK" : "FAILED",
         backend_guard ? "OK" : "FAILED");
    if (std::strlen(PHOTON_APP_ID) == 0u) {
        LOGW("16.1.1-photon: build has no PHOTON_APP_ID; online route is "
             "diagnostic passthrough until the Actions secret is configured");
    }
